...
export default function SearchPage() {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      if (selectedSpecialties.length) params.append("specialties", selectedSpecialties.join(","));

      const res = await fetch(`/api/doctors?${params.toString()}`);
      let doctors = (await res.json()).data || [];

      // Filter out doctors with missing essential details
      doctors = doctors.filter((doc: any) =>
        doc.id && doc.name && doc.experience && doc.qualification && doc.email && doc.phone && doc.bio
      );

      // Remove duplicates
      doctors = doctors.filter(
        (doc: any, index: number, self: any[]) => index === self.findIndex(d => d.id === doc.id)
      );
      ...
      if (selectedFilters.includes("availability")) {
        ...
        const availabilityPromises = doctors.map(async (doctor: any) => {
          const availRes = await fetch(`/api/doctors/${doctor.id}/availability?date=${today}`);
          const availData = await availRes.json();
          return { ...doctor, available: availData.data?.availableSlots?.length > 0 };
        });
        doctors = await Promise.all(availabilityPromises);
        doctors = doctors.filter((d: { available: any }) => d.available);
      }

      // Filter by rating
      if (selectedFilters.includes("rating")) {
        doctors = doctors.filter((d: { rating: number }) => d.rating >= 4);
      }

      // Filter by experience
      if (selectedFilters.includes("experience")) {
        doctors = doctors.filter((d: { experience: number }) => d.experience >= 10);
      }

      // Filter by insurance
      if (selectedFilters.includes("insurance")) {
        doctors = doctors.filter((d: { insuranceAccepted: any }) => d.insuranceAccepted);
      }
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-2">
              <button
                ...
                className="px-3 py-1 text-gray-700 hover:text-blue-600 transition-colors"
              >
                My Bookings
              </button>
              <button
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t">
            <nav className="flex flex-col p-4 space-y-2">
              <button onClick={handleNavBookings} className="text-gray-700 hover:text-blue-600 text-left">
                My Bookings
              </button>
              <button onClick={handleNavProfile} className="text-gray-700 hover:text-blue-600 text-left">
                Profile
              </button>
              <button onClick={handleLogout} className="text-red-600 hover:text-red-700 text-left">
                Logout
              </button>
            </nav>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-col lg:flex-row gap-4">
          <input
            ...
            className="flex-1 pl-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
        </div>
                ...
                {searchResults.map((doctor) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-16 h-16 sm:w-20 sm:h-20 bg-blue-100 rounded-full flex items-center justify-center text-xl text-blue-600 font-semibold">
                        {doctor.avatar || "D"}
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          <Link
                            href={`/ui/profiles?doctorId=${doctor.id}`}
                            className="border border-gray-300 hover:bg-gray-50 text-gray-700 px-3 py-1 rounded-lg font-medium"
                          >
                            View Profile
                          </Link>
                        </div>
